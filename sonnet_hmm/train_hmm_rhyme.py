...
def generate(lines, n_lines, section_type, num_poems):
	...
	np.savetxt(section_type+'_transition.txt', A)
	np.savetxt(section_type+'_start.txt', A_start)

	final_poem_list = []

	for counter in range(num_poems):
		...
		while line_cnt < num_lines_to_sample:
			...
			while True:
				...
				new_syll_count = count_syllables(str(sample_word) + ' ' + line)
		...
		final_poem_list.append(final_poem)

	return final_poem_list
